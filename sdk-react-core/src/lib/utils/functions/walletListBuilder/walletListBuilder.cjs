...
const sortedInputList = (inputList) => {
    ...
    inputList.forEach((wallet) => {
        if (PRIORITY_WALLET_LIST.includes(walletConnectorCore.normalizeWalletName(wallet.name))) {
    ...
    const sortedPriorityWallets = unsortedPriorityWallets
        .slice()
        .sort((a, b) => PRIORITY_WALLET_LIST.indexOf(walletConnectorCore.normalizeWalletName(a.name)) -
        PRIORITY_WALLET_LIST.indexOf(walletConnectorCore.normalizeWalletName(b.name)));
