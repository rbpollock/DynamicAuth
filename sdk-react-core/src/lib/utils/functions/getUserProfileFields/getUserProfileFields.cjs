 ...
 */
const getEditableUserProfileFields = (projectSettings) => index.getEnabledKYC(projectSettings).filter(({ name }) => !filterFields.includes(name));
/**
...
const getUserProfileFields = ({ projectSettingsKyc = [], user, }) => {
    ...
    const hasEmbeddedWallet = (_a = user === null || user === void 0 ? void 0 : user.verifiedCredentials) === null || _a === void 0 ? void 0 : _a.find(({ walletProvider }) => walletProvider === sdkApi.WalletProviderEnum.EmbeddedWallet);
