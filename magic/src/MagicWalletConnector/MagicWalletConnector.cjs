...
class MagicWalletConnector extends walletConnectorCore.WalletConnectorBase {
    constructor(opts) {
        ...
        (_e = this.chainRpcProviders) === null || _e === void 0 ? void 0 : _e.registerChainProviders(rpcProviders.ProviderChain.EVM, (config) => {
            const rpcProviders = {};
    ...
    getBalance() {
        return _tslib.__awaiter(this, void 0, void 0, function* () {
            ...
            if (!address)
                return;
            return viem.formatEther(yield provider.getBalance({ address }));
        });
    }
    ...
    getSignerSync(account) {
        ...
        if (!provider)
            throw this.generateDynamicErrorOnMissingProvider();
        const transport = viemUtils.createTransportWithUiConfirmation({
            connector: this,
            publicClient: provider,
            transport: viem.custom(client.rpcProvider),
    ...
    generateDynamicErrorOnMissingProvider() {
        return new utils.DynamicError(`No provider found for the current configured magic network.
...
exports.storedAddressKey = storedAddressKey;
